...
DEFAULT_SPARK_CONFIG: dict[str, Any] = {
	...
	"streaming": {
		"silver_trigger_seconds": 5,
		"gold_trigger_seconds": 5,
	},
	"gold": {
		"window_seconds": 60,
...
def _resolve_storage_path(path_text: str, default: str) -> str:
	resolved_text = path_text.strip() or default
	storage_path = Path(resolved_text)
	if storage_path.is_absolute():
		return str(storage_path)

	return str((PROJECT_ROOT / storage_path).resolve())
...
def load_spark_config(path: str | Path = "configs/spark.yaml") -> dict[str, Any]:
	...
	merged = {
		...
		"streaming": dict(DEFAULT_SPARK_CONFIG["streaming"]),
	...
	for section in ("spark", "streaming", "gold", "delta", "checkpoints", "kafka"):
...
def get_silver_trigger_seconds(default: int = 5) -> int:
	value = _section("streaming").get("silver_trigger_seconds", default)
	try:
		seconds = int(value)
...
def get_gold_trigger_seconds(default: int = 5) -> int:
	streaming_value = _section("streaming").get("gold_trigger_seconds")
	if streaming_value is not None:
		try:
			seconds = int(streaming_value)
			if seconds > 0:
				return seconds
		except (TypeError, ValueError):
			pass
	...
	try:
		minutes = int(_section("gold").get("cron_minutes", 0))
		if minutes > 0:
			return minutes * 60
	except (TypeError, ValueError):
		pass
...
def get_delta_path(layer: str, default: str) -> str:
	path = str(_section("delta").get(layer, default))
	return _resolve_storage_path(path, default)


def get_checkpoint_path(layer: str, default: str) -> str:
	path = str(_section("checkpoints").get(layer, default))
	return _resolve_storage_path(path, default)
